...
def index_page():
    ...
    return render_template('pages/termux_admin.html', token_file=_TOKEN_FILE)
...
def _cpu_info():
    ...
    首采样必然偏低（top 第一屏是自进程启动以来的累计均值），
    所以优先跑 ``-bn2`` 取第二个采样点，失败再退回单次采样。
    """
    info = {'usage': None, 'cores': os.cpu_count(), 'raw': ''}
    rc, out, _ = _run(['top', '-bn2', '-d', '1'], timeout=18)
    if rc != 0 or '%cpu' not in out:
        rc, out, _ = _run(['top', '-bn1'], timeout=12)
    if not out:
        return info
    info['raw'] = '\n'.join(out.splitlines()[:6])
    hits = re.findall(r'([\d.]+)%cpu\b(.*)', out)
    if hits:
        total = float(hits[-1][0])           # 取最后一个采样点
        idle_m = re.search(r'([\d.]+)%idle', hits[-1][1])
        if total > 0 and idle_m:
            idle = float(idle_m.group(1))
            info['usage'] = round(max(0.0, (total - idle)) / total * 100.0, 1)
    return info
